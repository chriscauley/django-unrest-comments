riot.tag2('comment', '<div class="comment_meta"> <a href="javascript:;" onclick="{collapse}" class="expand-link"></a> <span class="commented_by">{username} - </span> <span class="commented_date">{date_s}</span> </div> <div class="comment_content">{comment}</div> <div class="comment_actions"> <div if="{uR.auth.user}"> <a onclick="{reply}" title="reply" href="#"><i class="fa fa-reply"></i> Post Reply</a> <a if="{user_pk == uR.auth.user.id}" onclick="{edit}" title="reply" href="#"><i class="fa fa-pencil"></i> Edit</a> <a if="{window._418}" href="/admin/mptt_comments/mpttcomment/{pk}/delete/"><i class="fa fa-close"></i> Delete</a> </div> <div if="{!uR.auth.user}"> <a href="/accounts/login/?next={window.location.pathname}">Login to reply to this comment</a> </div> </div> <div class="comment_form"></div> <div class="comment_children"> <comment each="{comments}"></comment> </div>', '', '', function(opts) {
  var that = this;
...
riot.tag2('comment-form', '<form action="{opts.form_url}" method="POST" class="comment_form_wrapper" id="{opts.form_id}"> <div class="comment_form"> <md-help></md-help> <fieldset> <ul class="list-unstyled"> <li class="required"> <textarea cols="40" id="id_comment" name="comment" rows="10">{opts.comment}</textarea> <input id="id_content_type" name="content_type" type="hidden" riot-value="{opts.content_type}"> <input id="id_object_pk" name="object_pk" type="hidden" riot-value="{opts.object_pk}"> <input id="id_parent_pk" name="parent_pk" type="hidden" riot-value="{opts.parent_pk}"> <input id="id_comment_pk" name="comment_pk" type="hidden" riot-value="{opts.pk}"> </li> </ul> <input type="submit" class="submit-post btn btn-primary" value="Post" onclick="{submit}"> <input type="submit" class="submit-post btn btn-danger" value="Cancel" onclick="{cancel}"> </fieldset> </div> </form>', '', '', function(opts) {
  var self = this;
